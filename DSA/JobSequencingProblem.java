package DSA;

import java.util.Arrays;

public class JobSequencingProblem {
    
    /**
     * Given a set of n jobs where each jobi has a deadline and profit associated with it.

        Each job takes 1 unit of time to complete and only one job can be scheduled at a time. We earn the profit associated with a job if and only if the job is completed by its deadline.

        Find the number of jobs done and the maximum profit.

        Note: Jobs will be given in the form (Jobid, Deadline, Profit) associated with that Job. Deadline of the job is the time on or before which job needs to be completed to earn the profit.

        Examples :

        Input: Jobs = [[1,4,20],[2,1,1],[3,1,40],[4,1,30]]
        Output: 2 60
        Explanation: Job1 and Job3 can be done with maximum profit of 60 (20+40).


        Input: Jobs = [[1,2,100],[2,1,19],[3,2,27],[4,1,25],[5,1,15]]
        Output: 2 127
        Explanation: 2 jobs can be done with maximum profit of 127 (100+27).
     */
}






// class Solution
// {
//     //Function to find the maximum profit and the number of jobs done.
//     int[] JobScheduling(Job arr[], int n)
//     {
//         // Your code here
//         Arrays.sort(arr, (a, b) -> Integer.compare(b.profit, a.profit));
//          int maxDeadline = 0;
          
//         for (Job job : arr) {
//             maxDeadline = Math.max(maxDeadline, job.deadline);
//         }
        
//         int []ans = new int [maxDeadline +1];
//         Arrays.fill(ans,-1);
        
//         int profit =0;
//         int jobs=0;
        
//         for(int i =0; i<n; i++){
//             for(int j=Math.min(maxDeadline,arr[i].deadline); j>0; j--){
//                 if(ans[j]==-1){
//                     ans[j]= i;
//                     jobs++;
//                     profit+=arr[i].profit;
//                     break;
//                 }
//             }
//         }

//         return new int[]{jobs,profit};
//     }
// }






// more optimized

// class Solution {
//     // Function to find the maximum profit and the number of jobs done.
//     int[] JobScheduling(Job arr[], int n) {
//         // Sorting jobs by decreasing order of profit
//         Arrays.sort(arr, (a, b) -> Integer.compare(b.profit, a.profit));

//         // Find the maximum deadline to create the timeline array
//         int maxDeadline = 0;
//         for (Job job : arr) {
//             maxDeadline = Math.max(maxDeadline, job.deadline);
//         }

//         // Initialize a timeline to track scheduled jobs (up to max deadline)
//         boolean[] scheduled = new boolean[maxDeadline + 1];

//         int totalProfit = 0, totalJobs = 0;

//         // Iterate through jobs, scheduling the highest-profit ones first
//         for (int i = 0; i < n; i++) {
//             // Find the latest available slot before or on job's deadline
//             for (int j = Math.min(maxDeadline, arr[i].deadline); j > 0; j--) {
//                 if (!scheduled[j]) {
//                     // Schedule the job
//                     scheduled[j] = true;
//                     totalJobs++;
//                     totalProfit += arr[i].profit;
//                     break;
//                 }
//             }
//         }

//         return new int[]{totalJobs, totalProfit};
//     }
// }
